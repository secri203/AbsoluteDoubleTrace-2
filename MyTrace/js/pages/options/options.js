...
let Opts = {
				...
				bg.Stats.MainText(function(protectDate, stats, wrcData){
					...
					if (neatNumber(wrcData.listVer) === "0"){
						if (neatNumber(wrcData.listData.total) !== "0"){
							text += "<br />Trace 插件已经阻断了 <span>" + neatNumber(wrcData.listData.total) + "</span> 条记录。";
						}
					} else {
						if (neatNumber(wrcData.listData.total) !== "0"){
							text += "<br />Trace 插件已经从" + (wrcData.listOnline === 3 ? "已缓存的 " : "未缓存的") + (wrcData.listType == 'Built in' ? '内置' : wrcData.listType) + "阻断列表中，成功阻断了 <span>" + neatNumber(wrcData.listData.total) + "</span> 条记录。 <br />";

							try{
								text += "该阻断列表包含: ";
								text += (wrcData.listData.domain !== 0 ? neatNumber(wrcData.listData.domain) + "条域名， " : "");
								text += (wrcData.listData.host !== 0 ? neatNumber(wrcData.listData.host) + "条主机名， " : "");
								text += (wrcData.listData.tld !== 0 ? neatNumber(wrcData.listData.tld) + "条TLD， " : "");
								text += (wrcData.listData.url !== 0 ? neatNumber(wrcData.listData.url) + "条URL 和 " : "");
								text += (wrcData.listData.file !== 0 ? neatNumber(wrcData.listData.file) + "条tracking脚本。" : "");
							} catch(e){}

							text += "<br />WebController清单版本号： " + wrcData.listVer + "。";
						} else {
							text += "<br />域名阻断当前为启用状态。";
						}
					}
	...
	Premium:{
			...
			TraceBg(function(bg){
				...
				if (typeof(code) !== "string" || code === ""){
					...
					$("#premium_status, #info_premium_status").empty().append(
						$("<span/>",{class:"premium_inner"}).text("支持 Trace 的开发！购买高级版以获取高级阻止列表的访问权限。"),
						$("<br/>"),$("<br/>"),
						$("<button/>").on("click enter",Opts.Premium.EnterCode).text("输入激活码"),
						" ",
						$("<button/>").on("click enter",function(){
							openNewTab("https://dk.cyababy.com/buy/12");
		...
		RemoveCode:function(){
			TraceBg(function(bg){
				if (!confirm("您确定要从 Trace 中移除您的高级版激活码吗？\n\n您当前的激活码为：\n" + bg.Vars.Premium)) return;
		...
		EnterCode:function(){
			...
			$("#drop_message").empty().append(
				...
				$("<br />"),$("<br />"),
				$("<button/>",{"title":"激活高级版"}).text("激活").on("click enter",Opts.Premium.Go),
				$("<button/>",{"title":"我需要帮助"}).text("帮助").on("click enter",Opts.Premium.HelpDialog),
				Overlay.CloseUI()
			);
			Overlay.AssignCloseEvent(true);
		},
		HelpDialog:function(){
			$("#drop_message").empty().append(
				...
				$("<h2/>").text("如需要获取帮助，请联系vx：qf20210110"),
				Overlay.CloseUI()
			);
			Overlay.AssignCloseEvent(true);
		},
		Go:function(){
			...
			let u = "https://auth.cyababy.com/api/code_pay2";
			//u += btoa(eden);
			//u += "&s=" + btoa(lost);
			...
			case "pref_netpredict":
				...
				chrome.privacy.network.networkPredictionEnabled.get({},function(details){
					...
					if (change){
						...
						$(obj).text((details.value === true ? "未开启保护" : "已开启保护"));
					}
				});
				break;
	...
	Stats:{
		...
		MakeData:function(dataset,cb){
			...
			if (dLen === 0){
				...
				if (typeof chrome.extension.getBackgroundPage().Prefs.Current.Main_Trace === "undefined"){
					$("#graph_container").html("Error.<br /><br />请尝试重新启动您的浏览器。<br />").css("height","auto");
					return;
				}
				if (chrome.extension.getBackgroundPage().Prefs.Current.Main_Trace.ProtectionStats.enabled === true){
					$("#graph_container").html("统计功能已启用，但尚无可用数据。<br /><br />尝试浏览一下网页，然后再回来查看！<br />").css("height","auto");
				} else {
		...
		DeleteStats:function(){
			$("#drop_message").empty().append(
				$("<h1/>").text("删除统计信息"),
				$("<h3/>",{"class":"bold"}).text("这将删除所有保存的统计信息，但不会删除主页上的信息。"),
				$("<select/>",{id:"stats_del_amount"}).append(
					$("<option/>",{value:0}).text("保留今天的统计信息"),
					$("<option/>",{value:1}).text("保留过去 2 天的统计信息"),
					$("<option/>",{value:2}).text("保留过去 3 天的统计信息"),
					$("<option/>",{value:3}).text("保留过去 4 天的统计信息"),
					$("<option/>",{value:4}).text("保留过去 5 天的统计信息"),
					$("<option/>",{value:5}).text("保留过去 6 天的统计信息"),
					$("<option/>",{value:6}).text("保留过去一周的统计信息"),
					$("<option/>",{value:7}).text("删除所有统计信息")
				),
				$("<br/>"),$("<br/>"),
